...
class Upsonic_Update:
    def __init__(
        self, cloud, pre_update_all=False, pre_update_all_exclude=[], clear_olds=False
    ) -> None:
    ...
    def pre_update_all(self, pre_update_all_exclude=[]):
        ...
        console.log(f"[bold white] Preparing to Update:")
        for key in track(
            self.cloud.get_all(), description="           ", console=console
        ):
    ...
    def update(self, just_important: bool = False) -> bool:
        ...
        for key in track(
            self.pre_update_dict, description="           ", console=console
        ):
            result = False
